...
class bookingService {
    async addToCart(req: Request<{}, {}, reqBody>, res: Response): Promise<void> {
        try {
            const { productId } = req.body
            ...
            let cart = cartResponse?.data
            if (!cart) {
                const newCart = await create("Cart", { userId, items: [] })
                cart = newCart?.data
            }
            const existingItem = cart.items.find((item: any) => item.productId === productId);
            if (existingItem) {
    ...
    async placeOrder(req:Request<{},{},reqBody>,res:Response): Promise<void>{
        try {
            const {  address } = req.body;
            const userId = req.userId;
